...
function verifyAddon(addonPath, expectedVersion) {
  ...
  const actualVideoVersion = addon.cm_videoTapVersion();
  if (actualVideoVersion !== expectedVersion) {
    throw new Error(
      `expected video tap API version ${expectedVersion}, got ${actualVideoVersion}`,
    );
  }
}
