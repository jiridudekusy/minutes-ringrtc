...
impl AudioTap {
    ...
    fn apply_pending_gap(&self, source: AudioTapSource, buffers: &mut Buffers) {
        ...
        let (discarded_timeline_samples, discarded_buffered_samples) =
            append_gap_silence_bounded(buffer, pending_gap, self.capacity_samples_per_source);
        *start_sample += discarded_timeline_samples;
        self.dropped_counter(source)
...
fn append_gap_silence_bounded(
    buffer: &mut VecDeque<i16>,
    sample_count: u64,
    capacity: usize,
) -> (u64, u64) {
    let retained_silence = sample_count.min(capacity as u64) as usize;
    let retained_buffer_capacity = capacity - retained_silence;
    let dropped_from_buffer = buffer.len().saturating_sub(retained_buffer_capacity);
    buffer.drain(..dropped_from_buffer);
    buffer.extend(std::iter::repeat_n(0, retained_silence));

    let dropped_silence = sample_count - retained_silence as u64;
    (
        dropped_from_buffer as u64 + dropped_silence,
        dropped_from_buffer as u64,
    )
}
...
mod tests {
    ...
    #[test]
    fn lock_contention_preserves_buffered_pcm_and_materializes_silence() {
        ...
        assert_eq!(chunk.remote_playout, vec![1, 2, 0, 0, 5, 6]);
        assert_eq!(chunk.remote_playout_start_sample, 0);
        assert_eq!(chunk.dropped_remote_playout, 2);
    }

    #[test]
    fn a_contention_gap_remains_bounded_and_advances_the_timeline() {
        let tap = AudioTap::new(4);
        tap.start();
        tap.push(AudioTapSource::RemotePlayout, &[1, 2, 3]);

        let guard = tap.buffers.lock().expect("tap buffers");
        tap.push(AudioTapSource::RemotePlayout, &[4, 5, 6, 7, 8, 9]);
        drop(guard);
        tap.push(AudioTapSource::RemotePlayout, &[10]);

        let chunk = tap.drain(4);
        assert_eq!(chunk.remote_playout, vec![0, 0, 0, 10]);
        assert_eq!(chunk.remote_playout_start_sample, 6);
        assert_eq!(chunk.dropped_remote_playout, 10);
    }
}
